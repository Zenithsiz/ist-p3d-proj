...
Plane::Plane(Vector &a_PN, float a_D) : PN(a_PN), D(a_D) {}
	...
	if (file >> cmd) {
		while (true) {
			...
			} else if (cmd == "p") // Polygon: just accepts triangles for now
			{
				...
				for (unsigned int i = 0; i < total_faces; i++) {
					...
					triangle = new Triangle(
						verticesArray[P0], verticesArray[P1], verticesArray[P2]
					); // vertex index start at 1
					if (material) {
						triangle->SetMaterial(material);
					}
					this->addObject((Object *)triangle);
			...
			else if (cmd == "light") // Need to check light color since by default is white
			{
				...
				// Create Camera
				camera =
					new Camera(from, at, up, fov, hither, 1000.0 * hither, xres, yres, aperture_ratio, focal_ratio);
...
void Scene::create_random_scene() {
	...
	camera = new Camera(
		Vector(-5.312192, 4.456562, 11.963158),
		Vector(0.0, 0.0, 0),
		Vector(0.0, 1.0, 0.0),
		40.0,
		0.01,
			...
			if ((center - Vector(4.0, 0.2, 0.0)).length() > 0.9) {
				if (choose_mat < 0.4) { // diffuse
					material = new Material(
						Color(rand_double(), rand_double(), rand_double()), 1.0, Color(0.0, 0.0, 0.0), 0.0, 10, 0, 1
					);
					...
					material = new Material(
						Color(0.0, 0.0, 0.0),
						0.0,
						Color(rand_double(0.5, 1), rand_double(0.5, 1), rand_double(0.5, 1)),
						1.0,
						220,
						0,
						1
					...
					material = new Material(
						Color(rand_double(0.6, 1), rand_double(0.6, 1), rand_double(0.6, 1)),
						0.0,
						Color(1.0, 1.0, 1.0),
						0.7,
						20,
