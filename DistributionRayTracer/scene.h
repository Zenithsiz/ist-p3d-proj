...
class Light {
  ...
  public:
	...
	Light(const Vector &pos, const Color &col) {
		type = PUNCTUAL;
		position = pos; // position of point light or the center in the area light
		emission = col;
	}
...
class Scene {
  public:
	...
	accelerator GetAccelStruct() {
		return accel_struc_type;
	}

	void SetBackgroundColor(Color a_bgColor) {
		bgColor = a_bgColor;
	}
	void SetSkyBoxFlg(bool a_skybox_flg) {
		SkyBoxFlg = a_skybox_flg;
	}
	void LoadSkybox(const char *);
	void SetCamera(Camera *a_camera) {
		camera = a_camera;
	}
	void SetAccelStruct(accelerator accel_t) {
		accel_struc_type = accel_t;
	}
